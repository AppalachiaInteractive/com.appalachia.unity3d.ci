...
namespace Appalachia.CI.Compilation
{
    internal static class ScriptingDefineManagerUI
    {
        ...
        {
            var provider =
                new SettingsProvider("Preferences/Appalachia/Scripting Defines", SettingsScope.User)
                {
        ...
        private static void DrawFiltered()
        {
            ...
            if (filtered)
            {
                PostLabel($"Assemblies that contain [{filterValue}] will be giving a scripting define.");
            }
            else
            {
                PostLabel(
